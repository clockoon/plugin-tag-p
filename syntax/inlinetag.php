 ...
 */
class syntax_plugin_tag_inlinetag extends DokuWiki_Syntax_Plugin {
    ...
    function handle($match, $state, $pos, Doku_Handler $handler) {
        $tags = trim(substr($match, 1));     // strip markup & whitespace
    ...
    function render($format, Doku_Renderer $renderer, $data) {
        ...
        if ($format == 'xhtml') {
            ...
            $renderer->doc .= '<span class="'.$this->getConf('tags_list_css').'_inline">'
                .$tags
                . '</span>';
            return true;
